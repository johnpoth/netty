 ...
 * @see DatagramPacket
 */
public final class NioDatagramChannelCopy
     ...
     */
    public NioDatagramChannelCopy(SelectorProvider provider) {
     ...
     */
    public NioDatagramChannelCopy(InternetProtocolFamily ipFamily) {
     ...
     */
    public NioDatagramChannelCopy(SelectorProvider provider, InternetProtocolFamily ipFamily) {
     ...
     */
    public NioDatagramChannelCopy(DatagramChannel socket) {
...
//      RecvByteBufAllocator.Handle allocHandle = unsafe().recvBufAllocHandle();
        RecvByteBufAllocator.Handle allocHandleHeader = new FixedRecvByteBufAllocator(8).newHandle();
        ByteBuf dataHeader = allocHandleHeader.allocate(config.getAllocator());
        allocHandleHeader.attemptedBytesRead(dataHeader.writableBytes());
        ByteBuf data = null;
        boolean free = true;
        try {
            ByteBuffer nioDataHeader = dataHeader.internalNioBuffer(dataHeader.writerIndex(), dataHeader.writableBytes());
            int posHeader = nioDataHeader.position();
            InetSocketAddress remoteAddress = (InetSocketAddress) ch.receive(nioDataHeader);
            if (remoteAddress == null) {
                return 0;
            }

            allocHandleHeader.lastBytesRead(nioDataHeader.position() - posHeader);
            //length is in byte 5,6 and is in byte length
            byte[] array = new byte[8];
            dataHeader.readBytes(array);
            nioDataHeader.get(array);
            //packetSize in bytes minus the header size (8)
            int packetSize = (array[6]  & 0xFF) + ((array[6]  & 0xFF) << 8) - 8;
            
            //Read da rest
            RecvByteBufAllocator.Handle allocHandle = new FixedRecvByteBufAllocator(packetSize).newHandle();
            data = allocHandle.allocate(config.getAllocator());
            allocHandle.attemptedBytesRead(data.writableBytes());

            ByteBuffer nioData = data.internalNioBuffer(data.writerIndex(), data.writableBytes());
            int pos = nioData.position();
            remoteAddress = (InetSocketAddress) ch.receive(nioDataHeader);
            if (remoteAddress == null) {
                return 0;
            }
            ...
            CompositeByteBuf composite = config.getAllocator().compositeBuffer();
            composite.addComponent(data);
            composite.addComponent(dataHeader);
            buf.add(new DatagramPacket(composite,
                    localAddress(), remoteAddress));

//            buf.add(new DatagramPacket(dataHeader.writerIndex(dataHeader.writerIndex() + allocHandleHeader.lastBytesRead()),
//                localAddress(), remoteAddress));
